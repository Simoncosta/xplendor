...
import { fetchMe, login, registerByInvite } from '@/store/authSlice';

const ComponentsAuthRegisterForm = ({ token }: { token: string }) => {
    ...
    const submit = async (data: any) => {
        ...
        const result = await dispatch(registerByInvite({ password, password_confirmation, token }));
        await dispatch(fetchMe(result.payload.data));

        if (registerByInvite.fulfilled.match(result)) {
            router.push('/');
        }
    };
