...
>
> Última actualização: 2026-06-15 · Versão 1.13.4
...
57. **Distinguir `is_consignment` de `is_resume`** (descoberto via feedback Matilde, 2026-06-15 — **pré-requisito bloqueante** da faixa "Priorização de promoção + tráfego pago", sec 15). Hoje `cars.is_resume` (mapeado no glossário sec 19 e na API pública como `is_trade_in` desde 1.9.0) significa **retoma** — viatura entregue em troca pela compra de outra. **Consignação é semanticamente distinta** — viatura de particular vendida pelo stand à comissão, frequentemente sem propriedade do stock. Matilde avisou que consignações são precisamente as que pode **NÃO** querer promover (margem mais apertada, particular pode retirar a viatura a qualquer momento). Decisão a tomar com a Matilde antes da faixa: **(a)** campo novo `is_consignment` boolean em `cars` (migration reversível, condicional motorhome/caravan?); **(b)** estender o enum `condition` existente (tem `trade_in`, podia ter `consignment`); **(c)** sub-flag de `is_resume` (combinação `is_resume + is_consignment`). Opção (a) é a mais clara; (b) é sub-óptima porque `condition` é estado-físico, não modelo-de-venda. Não decidir sem ela.
...
- **Faixa "Priorização de promoção + tráfego pago"** (futura). Origem: dor real da operação — Matilde perguntada repetidamente "qual autocaravana promovemos?". **Duas features SEPARADAS, construídas em sequência:**
   1. **Lista de prioridades** (interno, sem Meta). Tela onde o stand marca que viaturas quer potenciar + define orçamento mensal por viatura (ex.: €100/mês até vender). **Tabela nova**, sem integração externa. Entrega valor sozinha — substitui o vai-e-vem de decisão "qual promovemos esta semana?". Orçamento começa em valor definido pela Matilde; sugestão automática (via IPS + dias em stock + posição no mercado) é fase posterior.
   2. **Ligação ao Meta Ads** (sobre a lista). Criar/pausar campanhas conforme a lista, pausar ao vender, acompanhar gasto real vs orçamento. **Usa o backend Meta INTACTO desde H3b** — `MetaAdsService`, OAuth, `company_integrations`, `FetchMetaAdsJob`. Reaproveita a infra existente que ficou em pé propositadamente quando a aba `/cars/:id/ads` foi removida do frontend (item 38 da dívida).
   - Construir (1) primeiro, validar com Matilde, só depois (2). Liga às ideias 1+2 da sessão inicial (motor de decisão + piloto de ads).
   - Serve directamente a faixa **Agência** (sec 2.1), não viola sec 16, funciona com 1 cliente.
   - **PRÉ-REQUISITO bloqueante** (item 57 da dívida técnica): definir se `is_consignment` é campo separado de `is_resume` ANTES de construir a tela de priorização — Matilde avisou que consignações são precisamente as que pode NÃO querer promover. Não confundir os dois conceitos.
