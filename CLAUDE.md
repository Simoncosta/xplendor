...
>
> Última actualização: 2026-06-18 · Versão 1.14.5
...
67. **🟢 Capacity da cama snap-back para 1 com Backspace (caso conhecido, baixa prioridade)** (auditoria pós Sub-fase F, 1.14.4, 2026-06-18). [`CarVehicleDetailsDataFields.tsx:380-398`](web/src/pages/Cars/Car/components/CarVehicleDetailsDataFields.tsx#L380-L398), o onChange do input `<Input type="number">` da capacity faz `parsed = v === "" ? 1 : Math.max(1, Math.min(4, Number(v)))`. Quando o utilizador limpa o campo com **Backspace** (em vez de double-click + escrever-por-cima), o handler força capacity a 1 imediatamente — React re-renderiza com `value={1}` e o cursor pula, frustrando o typing intermédio. **NÃO bate no fluxo típico:** double-click + escrever substitui directamente o "1" pelo "3" sem passar pelo estado vazio (Simon confirmou em browser, 2026-06-18: *"escrevo 3 e FICA"*). **Apenas afecta utilizadores que limpem com Backspace para depois escrever.** Fix possível: extrair sub-componente `BedCapacityInput` com state local da string raw (evita regras de hooks em loop + permite estado intermédio vazio sem snap-back) + commit ao Formik só com input válido + clamp final no blur. Custo desproporcionado para um sintoma que não morde no uso real — adiar até alguém reclamar. **Prioridade baixa.** Mesmo padrão das limitações conhecidas v1 da busca universal (dívidas 64 e 65): documentar, não atacar até haver reclamação concreta.
